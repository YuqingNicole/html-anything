...
export function renderReader(document) {
  ...
  return `<article class="document-reader ${templateClass}" data-document-id="${escapeAttr(document.id)}"><div class="reader-header"><p class="document-template">${meta.eyebrow}</p><span class="reader-status">ONE-MINUTE MODEL</span></div><section id="conclusion" class="reader-conclusion"><span class="conclusion-label">THE SHORT ANSWER</span><h2>${escapeHtml(document.title)}</h2><p class="document-thesis">${escapeHtml(document.thesis)}</p></section><div id="visual" class="reader-visual"><div class="reader-flow-label">THE SHAPE OF IT</div><div class="reader-flow">${flow}</div></div>${edges.length ? `<section id="relationships" class="relationship-section"><div class="section-label">WHAT CHANGES WHAT</div><ul class="document-relations">${edgesHtml}</ul></section>` : ''}<section id="insight" class="reader-insight"><span>HOW TO READ THIS</span><p>${meta.insight}</p></section><section id="explanation" class="explanation-section"><div class="section-label">UNPACK THE MODEL</div><div class="explanation-cards">${cards}</div></section>${sources ? `<p class="reader-sources"><span>FACT CHECK</span> Sources · ${sources}</p>` : `<p class="reader-sources"><span>NOTE</span> This is a simplified model. Add sources before treating it as a factual reference.</p>`}</article>`;
}
